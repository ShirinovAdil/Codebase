...
def show_code(request):
    ...
    paginator = Paginator(all_codes_previous, 3)
    try:
...
class AddExercise(CreateView):
    # Add new problem to list of problems
...
def code_details(request, pk=None):
    # List of users sent solutions, ADD SOLUTION
...
#     @method_decorator(login_required(redirect_field_name='accounts:login'))
#     def dispatch(self, *args, **kwargs):
#         return super(DeleteCode, self).dispatch(*args, **kwargs)
#     model = PersonalCode
#     success_url = reverse_lazy('Share:show_code')


# def edit_current_code(request, some=None, code_id=None):
#     # Edit sent code
#     instance = get_object_or_404(PersonalCode, pk=code_id)
#
#     if request.method == 'POST':
#         form = ShareForm(request.POST or None, instance=instance)
#         if form.is_valid():
#             instance = form.save(commit=False)
#             instance.save()
#             return redirect('Share:show_code')
#     else:
#         form = ShareForm(instance=instance)
#
#     context = {
#             'form': form
#         }
#     return render(request, 'share/add_code.html', context)
